...
# reload file
rm $TELEOP_CRED
cp ../credentials/aws-configuration.js $TELEOP_CRED
...
export accessKeyId_ani=`cat ../credentials/credentials | grep aws_access_key_id | cut -d \= -f2`
export secretAccessKey_ani=`cat ../credentials/credentials | grep aws_secret_access_key | cut -d \= -f2`
export sessionToken_ani=`cat ../credentials/credentials | grep aws_session_token | cut -d \= -f2`
