...
### Setup Credentials
Copy AWS Credentials to `~/environment/jetbot/assets/scripts/resources/credentials` and run following commands. Then download `~/environment/jetbot/assets/teleop' folder for controlling Robot with virtual joystick.
```
cd ~/environment/jetbot/assets/scripts
./set_credentials.sh
```
